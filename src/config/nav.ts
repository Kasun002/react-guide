export type NavItem = {
  label: string;
  href: string;
};

// Add or remove items here — the navbar handles any number automatically.
export const navItems: NavItem[] = [
  { label: "Home", href: "/" },
  { label: "Example 1", href: "/example-1" },
  { label: "Example 2", href: "/example-2" },
  { label: "Example 3", href: "/example-3" },
  { label: "Example 4", href: "/example-4" },
  { label: "Example 5", href: "/example-5" },
  { label: "Example 6", href: "/example-6" },
  { label: "Example 7", href: "/example-7" },
  { label: "Example 8", href: "/example-8" },
  { label: "Example 9", href: "/example-9" },
  { label: "Example 10", href: "/example-10" },
  { label: "Example 11", href: "/example-11" },
];
